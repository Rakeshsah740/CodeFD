...
Nx, Ny = 400, 400  # Square domain, Ny-> horizontal, Nx -> vertical
tau = 0.6          # Relaxation time
steps = 20001       # Number of iterations
plot_every = 100    # Plotting interval
u_top = 0.0833       # Lid velocity
...
for it in range(steps):
    ...
    import os
    os.makedirs('plots_u_top', exist_ok=True)  # Create folder only once at start
    ...
    if it % plot_every == 0:
        ...
        plt.savefig(f'plots_u_top/u_x_field_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.savefig(f'plots_u_top/u_y_field_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.savefig(f'plots_u_top/velocity_magnitude_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.savefig(f'plots_u_top/vorticity_field_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.savefig(f'plots_u_top/temperature_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(f'plots_u_top/ux_profile_step_{it:01d}.png', dpi=250)
        plt.close()
        ...
        plt.savefig(f'plots_u_top/streamlines_step_{it:01d}.png', dpi=100, bbox_inches='tight')
        plt.close()
        print("step =", it)
