...
def compute_error(ux, uy, ux_mid_prev, uy_mid_prev, Ny, Nx, eps=1e-12):
    ...
    error_ux_rel = np.max(np.abs(ux_mid - ux_mid_prev) / (np.abs(ux_mid_prev) + eps))
    error_uy_rel = np.max(np.abs(uy_mid - uy_mid_prev) / (np.abs(uy_mid_prev) + eps))
    ...
    return total_error_abs, total_error_rel, ux__mid_prev.copy(), uy__mid_prev.copy()
